`timescale 1ns/1ps
// SPIKE REDO: mesh_2x1_vc8 struct-port elaboration de-risk (throwaway).
//
// REDO reason: prior spike (746ea1f) used ni_signals_pkg::noc_credit_t
// which was credit[0:0] (NOC_NUM_VC_DFLT=1).  This redo uses
// noc_types_pkg::noc_credit_t from noc_types_pkg_vc8.sv, which is
// credit[7:0] (real 8-bit), to actually verify vc8 struct elaboration.
//
// Channel face: ni_signals_pkg::noc_chan_t (unchanged).
// Credit face:  noc_types_pkg::noc_credit_t (from noc_types_pkg_vc8.sv).
//
...
module node_req_driver (
    ...
    input  noc_types_pkg::noc_credit_t    cred_i,
...
module node_credit_echo (
    ...
    output noc_types_pkg::noc_credit_t    cred_o
);
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            cred_o.credit <= '0;
...
//   node_req      [NUM_NODES]  - ni_signals_pkg::noc_chan_t   (output) channel face
//   node_cred     [NUM_NODES]  - noc_types_pkg::noc_credit_t  (output) credit[7:0] vc8
...
module noc_fabric_spike #(
    ...
    output noc_types_pkg::noc_credit_t  node_cred [NUM_NODES],
    ...
    noc_types_pkg::noc_credit_t  w_cred [NUM_NODES];
