...
def checker_sv(plan, node_count, memory_targets):
    ...
    for node in range(node_count):
        ...
        if any(t.phase for t in issued):
            ...
            w(f"        if (!rst_ni) phase_retired_{node}_reg = 0;")
            w(f"        else if ({endpoint}.dma_job_rsp_valid) begin")
            ...
            w("    end")
        w("    always @(posedge clk_i) begin")
        w(f"        if (rst_ni && {endpoint}.dma_job_req_valid && {endpoint}.dma_job_req_ready) begin")
    ...
    w("        do begin")
    w("            @(posedge clk_i);")
    w("            complete = rst_ni && operation_done_reg;")
