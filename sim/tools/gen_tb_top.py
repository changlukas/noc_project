...
def _dma_check(topo, n_ep, jobs_per_node, job_bytes, rw):
    ...
    w("        int unsigned nsu_write_hwm, nsu_read_hwm;")
    w("        bad_total = 0;")
    w("        do begin")
    w("            @(posedge clk_i);")
    w("            all_done = rst_ni;")
    ...
    w("            cmodel_nsu_meta_buffer_hwm(nsu_ctx[i], nsu_write_hwm, nsu_read_hwm);")
    w('            $display("[NSU_HWM] node=%0d write_meta_hwm=%0d read_meta_hwm=%0d",')
    w("                     i, nsu_write_hwm, nsu_read_hwm);")
    w("        end")
...
def emit_tb_top(topo: dict, dma: bool = False,
    ...
    if dma:
        ...
        w("        timeout_cycles = TIMEOUT_BASE")
        w("            + K_CYC_PER_BEAT * expected_total * MAX_BURST_BEATS;")
    ...
    w('    import "DPI-C" context function void cmodel_perf_begin(input longint start_cyc);')
    ...
    w('                                                              input int port_id,')
    w('                                                              input string config_path);')
    w('    import "DPI-C" context function void cmodel_nsu_meta_buffer_hwm(')
    w('                                                                 input longint unsigned ctx,')
    w('                                                                 output int unsigned write_hwm,')
    w('                                                                 output int unsigned read_hwm);')
    ...
    if not dma:
        w(f"    int unsigned expected_txn_cnt [{n_ep}];")
        w(f"    int unsigned expected_write_cnt [{n_ep}];")
        w(f"    longint unsigned stimulus_start_cycle [{n_ep}];")
        w(f"    longint unsigned stimulus_done_cycle [{n_ep}];")
        w(f"    logic compare_ready [{n_ep}];")
        w("    logic compare_start;")
        w(f"    logic compare_done [{n_ep}];")
        w("    int unsigned injection_mode = 0;")
        w("    initial void'($value$plusargs(\"injection_mode=%d\", injection_mode));")
        w("    bit round_perf = 1'b0;")
        w("    initial void'($value$plusargs(\"round_perf=%d\", round_perf));")
        w("    always_comb begin")
        w("        compare_start = injection_mode == 3;")
        w(f"        for (int i = 0; i < {n_ep}; i++)")
        w("            compare_start &= compare_ready[i];")
        w("    end")
    if dma:
    ...
    if dma:
        ...
        w("            .end_of_sim_o(end_of_sim[i]), .txn_cnt_o(txn_cnt[i]),")
        w("            .expected_txn_cnt_o(expected_txn_cnt[i]),")
        w("            .expected_write_cnt_o(expected_write_cnt[i]),")
        w("            .stimulus_start_cycle_o(stimulus_start_cycle[i]),")
        w("            .stimulus_done_cycle_o(stimulus_done_cycle[i]),")
        w("            .compare_ready_o(compare_ready[i]), .compare_start_i(compare_start),")
        w("            .compare_done_o(compare_done[i])")
    w("        );")
    if dma:
    ...
    w("        bit all_done;")
    w("        int unsigned expected_total;")
    w("        int unsigned active_sources;")
    w("        int unsigned write_bursts;")
    w("        longint unsigned round_start;")
    w("        longint unsigned round_completion;")
    w("        int unsigned aw_idle, aw_same, aw_alloc, ar_idle, ar_same, ar_alloc;")
    w("        int unsigned list_hwm, wtxn_hwm, rtxn_hwm;")
    w("        int unsigned nsu_write_hwm, nsu_read_hwm;")
    ...
    w(f"        for (int i = 0; i < {exit_n}; i++) begin")
    w("            expected_total += expected_txn_cnt[i];")
    w("            if (expected_txn_cnt[i] > 0 && txn_cnt[i] == 0) begin")
    w("                vacuous = 1'b1;")
    w('                $display("FAIL: node%0d completed zero of %0d loaded transactions",')
    w("                         i, expected_txn_cnt[i]);")
    w("            end")
    w("        end")
    w('        if (expected_total == 0) $fatal(1, "tb_top: empty stimulus");')
    w('        if (vacuous) $fatal(1, "tb_top: vacuous run");')
    w("        active_sources = 0;")
    w("        write_bursts = 0;")
    w(f"        for (int i = 0; i < {exit_n}; i++) begin")
    w("            if (expected_write_cnt[i] > 0) active_sources++;")
    w("            write_bursts += expected_write_cnt[i];")
    w("        end")
    w('        $display("[TrafficMeta] active_sources=%0d write_bursts=%0d",')
    w("                 active_sources, write_bursts);")
    w("        if (round_perf) begin")
    w("            if (injection_mode != 0)")
    w('                $fatal(1, "RoundPerf requires injection_mode=0");')
    w("            round_start = 0;")
    w("            round_completion = 0;")
    w("            active_sources = 0;")
    w(f"            for (int i = 0; i < {exit_n}; i++) begin")
    w("                if (expected_txn_cnt[i] > 0) begin")
    w("                    if (expected_txn_cnt[i] != expected_write_cnt[i])")
    w('                        $fatal(1, "RoundPerf requires write-only stimulus at node%0d", i);')
    w("                    if (active_sources == 0)")
    w("                        round_start = stimulus_start_cycle[i];")
    w("                    else if (stimulus_start_cycle[i] != round_start)")
    w('                        $fatal(1, "RoundPerf sources did not start together");')
    w("                    if (stimulus_done_cycle[i] > round_completion)")
    w("                        round_completion = stimulus_done_cycle[i];")
    w("                    active_sources++;")
    w("                end")
    w("            end")
    w('            $display("[RoundPerf] start_cycle=%0d completion_cycle=%0d round_cycles=%0d active_sources=%0d write_bursts=%0d",')
    w("                     round_start, round_completion, round_completion - round_start,")
    w("                     active_sources, write_bursts);")
    w("        end")
