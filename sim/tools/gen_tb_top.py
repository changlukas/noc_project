...
def _dma_check(topo, n_ep, jobs_per_node, job_bytes):
    ...
    w("    // regions only -- a destination byte the DMA never wrote has no mem[] key")
    ...
    w("    // (axi_sim_mem.sv:177 then :182-187), so it is that endpoint's own")
    w("    // statement that the payload is in the array the compare reads. It is not")
    w("    // independent of jobs_retired -- the same B travels back and retires the")
    w("    // job -- so it adds a reading point, not coverage. The real check is the")
    w("    // compare: a byte the DMA never wrote has no mem[] key, reads 0, and")
    w("    // differs from its source's pattern.")
    w("    //")
