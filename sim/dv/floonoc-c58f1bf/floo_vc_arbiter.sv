...
// Ported from pulp-platform/FlooNoC @ c58f1bf13baeda147b4e87e961683d389db090a1.
// Credit mode: generalized VC count and single-VC accounting; pinned counter clr_i API.
...
if (NumVirtChannels == NumPhysChannels && VcImpl != VcCredit) begin : gen_virt_eq_phys
  assign valid_o = valid_i;
  assign ready_o = ready_i;
  assign data_o  = data_i;
end else if (NumPhysChannels == 1) begin : gen_single_phys

    typedef logic [(NumVirtChannels > 1 ? $clog2(NumVirtChannels) : 1)-1:0] arb_idx_t;
  ...
  `ASSERT_INIT(SupportedNumVirtChannels, (VcImpl != floo_pkg::VcPreemptValid) || (NumVirtChannels <= 2))
