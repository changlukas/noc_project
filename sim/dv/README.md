...
iDMA is the one package whose upstream paths differ from the `src/` + `include/` layout used here, so
the files column above records where each file sits, not where it came from. To reproduce the fetch:
`idma_pkg.sv` is upstream `src/`, the three `*_rw_axi.sv` are `target/rtl/`, the other six `.sv` are
`src/backend/`, and both headers are `src/include/idma/`.
