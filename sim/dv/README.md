...
|---------|----------|-----|-------|---------|----------|
| axi-0.39.7 | github.com/pulp-platform/axi | v0.39.7 | src/{axi_pkg,axi_intf,axi_test}.sv src/{axi_xbar,axi_xbar_unmuxed,axi_mux,axi_demux,axi_demux_simple,axi_err_slv,axi_atop_filter,axi_multicut,axi_cut,axi_id_prepend,axi_delayer,axi_sim_mem,axi_rw_join}.sv include/axi/{typedef,assign}.svh | Solderpad 0.51 | - |
...
| idma-0.6.5 | github.com/pulp-platform/iDMA | v0.6.5 | src/idma_pkg.sv src/include/idma/{typedef,guard}.svh target/rtl/idma_{backend,legalizer,transport_layer}_rw_axi.sv src/backend/idma_{axi_read,axi_write,dataflow_element,error_handler,channel_coupler,legalizer_page_splitter}.sv | Solderpad 0.51 | - |
...
the default `ErrorCap` elides it, so raising `ErrorCap` does not become a second vendoring round. Layout
is flattened: the RTL sits at the package root and the two headers at `idma/`, so one `-I` on the
package directory resolves `` `include "idma/guard.svh" ``. The five new common_cells files and
`axi_rw_join.sv` are pulled in by this closure and are taken at the tags already vendored here.
