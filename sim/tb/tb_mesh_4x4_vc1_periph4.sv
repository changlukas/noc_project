...
// Configuration: 4x4 mesh with four off-mesh peripherals, 1 DAT VC, NMU read
...
// evidence that all four peripherals were attached.
//
...
// the package already emits them in that layout. Reversing them here would
// attach every peripheral to the wrong router and still pass a symmetric-pattern
// scoreboard, so the order is not something a green run would catch.
//
// The module is named tb_top so --top-module needs no per-configuration value.

module tb_top;

    import topology_mesh_4x4_periph4_pkg::*;
