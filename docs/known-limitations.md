...
| The target permits `AXI_ID_WIDTH` 1..8 and derives REQ/RSP physical widths from it, but specgen generated AXI/flit/child types, DPI marshalling and C++ per-ID containers remain the fixed 3-bit, 136/126/633-bit instance. `AXI_ID_SPACE` has an explicit eight-entry static assertion | any customer configuration with `AXI_ID_WIDTH != 3`; the target interface cannot elaborate coherently until packet offsets, generated widths, wrappers and model containers are aligned together | `specgen/source/constants.yaml`; `specgen/generated/json/ni_packet.json`; `rtl/common/ni_child_types_pkg.sv`; `axi/types.hpp`, `AXI_ID_SPACE`; DPI and SV wraps |
