...
| The model is single-clock and does not implement the target five-channel AXI async-FIFO boundary (`NI_CDC_FIFO_DEPTH=8`). Its DAT allocators own per-VC pending queues, while target NI has no VC FIFO; its Router-to-NI LOCAL DAT path returns per-VC credits, while target ejection uses ready/valid. The model also defaults to one DAT VC rather than target two and implements only the `SHARED` candidate set; `READ_WRITE_SPLIT` masks are absent from NI allocation and router VA | cycle-exact CDC, target LOCAL DAT backpressure, Router-only VC ownership, or asynchronous AXI/NoC verification | `wrap/dat_merge_wrap.hpp`, `DatMergeWrap::tick`; `router/router.hpp`, `Router::vc_assignment`; `nmu/nmu_standalone.hpp`; `nsu/nsu_standalone.hpp`; `nmu/vc_allocator.hpp`; `nsu/vc_allocator.hpp` |
