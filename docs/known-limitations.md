...
| The target now separates external `AXI_ID_WIDTH` (1..8) from fixed 3-bit `NOC_ID_WIDTH`; production remap integration and non-default-width end-to-end evidence are still pending | before NMU/NSU top integration and focused 1/3/8-width remap tests close the contract | Issue #53; NMU/NSU top implementation |
