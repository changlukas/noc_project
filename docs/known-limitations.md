...
|---|---|---|
| The production-pinned `addr_decode` functional loop explicitly permits overlap and gives the highest array index priority, but its dynamic submodule's simulation-only `check_overlap` assumption and `more_than_1_bit_set` final assertion still report a legal multiple match when assertions are enabled | follow-on RTL/DV must provide a selective treatment for those two overlap diagnostics while keeping all other primitive and project assertions active; globally disabling assertions, changing the approved overlap policy, or replacing the decoder is not acceptable | production-pinned `addr_decode.sv` / `addr_decode_dync.sv`; `docs/verification-environment.md`, Provenance; `docs/nmu-verification-plan.md`, N0-SAM-06 |
...
| The C++ REQ/RSP `ready_slack` threshold is provisional and its exact deassertion cycle is not a target oracle | differential input-ready timing near almost-full occupancy | `router/simple_router.hpp`, `SimpleRouterConfig::ready_slack`; `docs/router-verification-plan.md`, R2-X06 |
...
| The target permits `AXI_ID_WIDTH` 1..8 and derives REQ/RSP physical widths from it, but specgen, DPI marshalling and C++ per-ID containers remain the fixed 3-bit, 136/126/633-bit instance. `AXI_ID_SPACE` has an explicit eight-entry static assertion | any customer configuration with `AXI_ID_WIDTH != 3`; the target interface cannot elaborate coherently until packet offsets, generated widths, wrappers and model containers are aligned together | `specgen/source/constants.yaml`; `specgen/generated/json/ni_packet.json`; `axi/types.hpp`, `AXI_ID_SPACE`; DPI and SV wraps |
