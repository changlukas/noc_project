...
| `docs/noc-target-spec.md` derives DAT deadlock freedom from read data landing in reorder-buffer space reserved at request issue, and a bypassed read reserves no slot, so the argument does not cover it. This predates the 3 b id round; what that round changed is the count. Removing the shared outstanding pool takes the `RobMode::Enabled` read bound from 32 to 256, worst case all 256 unreserved -- 8 ids each running a 32-deep same-destination bypass streak allocates no RoB slot at all. `RobMode::Disabled`, which a co-sim run now reaches only through `READ_ROB=0` and a ctest by setting `cfg.read_rob_mode` itself, moves the other way: `min(pool 32, per-id 256)` = 32 becomes 1 x 8 = 8 through the per-id single-outstanding interlock | in every build: `RobMode::Enabled` is the default the `NmuConfig` default constructor, `NmuWrap::init` and `cmodel_nmu_create` all take, and `READ_ROB` defaults to 1. Both conditions are now present in one run, and the cycle still did not form at that geometry. `gen_dma_jobs` emits read jobs sourced from another node's window, so reads cross the fabric: the DMA gate reports `read_txns_hwm=1` and `ar_clause={idle=2 same_dest=0 alloc=0}` on every node, against `read_txns_hwm=0` and every AR clause at zero before it. Condition one is observed rather than argued -- both ARs took the `ordering_req = 0` idle bypass and `read_slot_hwm` stayed 0, so neither reserved a slot. Condition two arrived with them: the endpoint holds R for 297 cycles on every node (`[mst_bp]`), where pulp's `axi_file_master` never stalls R at all, `wait_r` being a resident forked task that consumes a beat whenever `r_outst` is non-empty (`sim/dv/axi-0.39.7/src/axi_test.sv:2577-2586`). What the run does not establish is freedom: it is one topology at 4 jobs x 0x400 B per node, with 2 reads per node and one AXI ID | `nmu/rob.hpp`, the `ordering_req = 0` branches of `push_ar`; `sim/tools/gen_tb_top.py`, `READ_ROB_ENABLED` |
