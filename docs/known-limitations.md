...
| A peripheral outside the tile region on x exchanges requests and responses with its own row only. XY routing resolves x before y, so a request to it and the response for a request from it both run out of x hops on the row they started from; the one that misses lands in whichever peripheral borders that row, whose NSU rebases the address to its own tile and answers it | any transaction pairing such a peripheral with a node on another row. Refused at packetize time in both directions, so it costs reachability rather than correctness | `nmu/addr_trans.hpp`, `check_dst_reachable` |
