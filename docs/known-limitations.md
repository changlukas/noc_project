...
| `routing.use_id_table: false` is validated, not implemented: the loader checks the map would be legal under one global coordinate range pair, then every lookup still range-matches the SAM | never at the AXI boundary, since on a map meeting spec 5.1 both modes reach the same node at the same node-local offset. It bites at RTL handover, where the two are different hardware: 2N range compares against one bit slice | `nmu/sam_yaml.hpp`, `check_offset_ranges` |
...
| `--dma` on a topology carrying a peripheral | `gen_dma_jobs.job_table` walks the router array only, so a peripheral endpoint has no `jobs.txt` and its `idma_job_driver` `$fatal`s on the missing file. Refused at generate time: `gen_tb_top.main()` rejects `--dma` on a topology declaring `address_map.peripherals` and cites this row |
