...
| The simulator segfaults about once in ten runs, inside `axi_file_master::load_files` | Localised 2026-08-11 with `stdbuf -o0`: the failing run reaches 0 of 16 `[HWM]` lines and 0 monitor lines, and its last output is inside `load_files` at time 0, so it dies while the sixteen file masters open their stimulus files. Every earlier log ended in the same place for a different reason -- redirected stdout is block-buffered, so 4 KB of `load_files` output was simply all that had flushed, and the crash point could not be read off it. Same binary, same arguments, rc 139 on run 8 of 10 and 0 on the other nine. Not the admission telemetry, which never executes. Not memory exhaustion: 12.4 GB free |
