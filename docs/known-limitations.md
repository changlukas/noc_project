...
| `docs/noc-target-spec.md` derives DAT deadlock freedom from read data landing in reorder-buffer space reserved at request issue, and a bypassed read reserves no slot, so the argument does not cover it. This predates the 3 b id round; what that round changed is the count. Removing the shared outstanding pool takes the `RobMode::Enabled` read bound from 32 to 256, worst case all 256 unreserved -- 8 ids each running a 32-deep same-destination bypass streak allocates no RoB slot at all. `RobMode::Disabled`, which a co-sim run now reaches only through `READ_ROB=0` and a ctest by setting `cfg.read_rob_mode` itself, moves the other way: `min(pool 32, per-id 256)` = 32 becomes 1 x 8 = 8 through the per-id single-outstanding interlock | in every build: `RobMode::Enabled` is the default the `NmuConfig` default constructor, `NmuWrap::init` and `cmodel_nmu_create` all take, and `READ_ROB` defaults to 1. What still keeps the cycle out of reach is the second condition -- forming it needs a consumer that backpressures R, which this testbench has not got: pulp's `axi_file_master` never stalls its R channel, `wait_r` being a resident forked task that consumes a beat whenever `r_outst` is non-empty (`sim/dv/axi-0.39.7/src/axi_test.sv:2577-2586`). Stage B does NOT close it. The iDMA endpoint's read port would stall R when its write port is congested, but the shipped job stimulus has each node read its OWN memory window (`gen_dma_jobs.job_table`, `src_cid` is the issuing node's coordinate), which the tile crossbar answers at m0/m1, so no read reaches the NMU at all: a DMA gate run reports `read_txns_hwm=0` and every AR admission clause at zero on every node. Closing it needs jobs sourced from another node's window, which nothing emits | `nmu/rob.hpp`, the `ordering_req = 0` branches of `push_ar`; `sim/tools/gen_tb_top.py`, `READ_ROB_ENABLED` |
| `noc-target-spec.md` 5.1 requires a collective's request address to name a region the same size as its members', and nothing checks it. `collective_translate` measures the burst footprint against the region the address names and takes that as standing for every replica; the uniform-aperture walk now covers the tile region only, so a peripheral may declare a different size | a collective whose address names a region bigger than a tile: the footprint is measured against the wrong bound and an over-long burst reaches the fabric. No shipped topology declares a non-tile-sized region | `nmu/addr_trans.hpp`, `collective_translate`; `SamTable::declare_space_coords`, the aperture walk |
...
| `--dma` on a topology carrying a peripheral | `gen_dma_jobs.job_table` walks the router array only, so a peripheral endpoint has no `jobs.txt` and its `idma_job_driver` `$fatal`s on the missing file. Elaborates, cannot run |
