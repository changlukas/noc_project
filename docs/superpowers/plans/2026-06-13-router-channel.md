...
TEST(CreditRelay, DecrementThenRelayRestoresUpstreamCredit) {
    SCENARIO("CreditRelay: after the upstream output credit is spent, relay.receive_credit restores it");
    using ni::cmodel::noc::EjectAdapter;
    ...
    // Push a flit destined (0,0): it grants out the WEST output, spending one WEST credit.
    ASSERT_TRUE(inj.push_flit(req_flit(/*dst=*/0x00, /*vc=*/0)));
    inj.on_tick(); up.tick();   // stage1
    up.tick();                  // stage2: grant -> WEST output FIFO, WEST credit-- 
    EXPECT_EQ(up.credit(WEST, 0), seed - 1) << "WEST output credit spent on grant";
    relay.receive_credit(0);    // the downstream's EAST input freed -> relay restores WEST
...
TEST(RouterChannel, SingleFlitRspEndToEnd) {
    SCENARIO("RouterChannel: a RSP flit injected at NSU(0,0) ejects at NMU(1,0) (spec §7 both nets)");
    using ni::cmodel::noc::RouterChannel;
    RouterChannel ch(/*num_vc=*/2);
    Flit f = req_flit(/*dst=*/0x01, /*vc=*/0);   // dst=(1,0); helper just sets dst/vc/last
...
```
(`req_flit` already exists from Task 1; it only stamps dst/vc/last, reused for RSP flits too.)
...
class RouterChannel {
  public:
    ...
    // Test introspection (for the credit-conservation asserts in Task 4).
...
TEST(RouterChannel, EjectBoundaryCreditConservation) {
    SCENARIO("RouterChannel: req_router(0).credit(LOCAL,vc) + req_eject_buffered(0) == vc_depth every tick");
    using ni::cmodel::noc::RouterChannel;
    constexpr std::size_t kDepth = NI_NOC_ROUTER_VC_DEPTH;
    RouterChannel ch(/*num_vc=*/2, /*vc_depth=*/kDepth);
    const auto LOCAL = static_cast<std::size_t>(RouterPort::LOCAL);
    // Drive sustained requests to dst=(0,0); pop intermittently so the eject side
    // both fills and drains. The eject credit loop must conserve: the router's
    // LOCAL-output credit (spent on eject, returned on pop) plus the eject buffer
    // occupancy is invariant at the seed. Asserted EVERY tick (spec §7 required).
    for (int t = 0; t < 300; ++t) {
...
For Flow B's address→dst mapping (concrete, to avoid BLOCKED): `nmu::addr_trans::xy_route`
puts X in bits `[16 + X_WIDTH - 1 : 16]` (LOCAL_ADDR_BITS=16), so `dst_x=1` needs bit 16 set
→ add `0x10000` to each transaction address. Mechanism, in priority order (read
`c_model/include/axi/scenario_parser.hpp` first to pick):
1. If `ScenarioConfig` / `AxiMasterT` exposes an address base/offset, set it to `0x10000` for
   Flow B (Flow A uses 0).
2. Otherwise, after the scenario is loaded, add `0x10000` to each transaction's `addr` field
   in the parsed `ScenarioConfig` before constructing the master (the parsed struct is
   in-memory and mutable; this needs no YAML rewrite).
3. Last resort: write a shifted copy of the YAML to `TEST_TMPDIR` and point Flow B at it.
Use the lowest-numbered option the code supports. Verify a Flow B request's `dst_id == 0x01`
in the first end-to-end pass (assert it once) so a wrong offset fails loudly rather than
silently routing to the wrong node.
