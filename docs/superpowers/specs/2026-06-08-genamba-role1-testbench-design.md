...
- Simulator: Verilator 5.040 `--timing --binary`. Windows build via PowerShell driver — `--binary` link needs `sh.exe` (`C:\msys64\usr\bin`); PowerShell sets PATH + `LC_ALL=C` + `PYTHON3=py`.
...
- PowerShell-driven `make + run` shell needed on Windows; Bash sandbox blocks `cc1` for `--binary` link.
...
`tb_genamba` is a self-clocked SV top built `--binary --timing`:
...
- Does **not** reuse `main.cpp` — main.cpp's C++-driven clock would conflict with gen_amba's SV-timed BFM.
...
| `cosim/verilator/run_genamba.ps1` (Windows driver) | NEW | This effort |
...
| B | `test_burst_blen{4,8,16}` | Call vendored `axi_master_write(addr, bnum=16, bleng=blen, burst=2'b01 /*INCR*/, lock=1'b0, delay=0)` then `axi_master_read(...)` over distinct addrs; record expected per address; manual SV compare in wrapper. Signature per `axi_master_tasks.v:186`: `(addr, bnum, bleng, burst, lock, delay)` — no `instance` argument. | AXI INCR burst at `blen ∈ {4, 8, 16}`; (AXI4 INCR allows up to 256; gen_amba primitive caps at 16 — we sweep up to that cap) |
| C | `test_outstanding_N{4,8}` (parameterized fixture) | **Do not use** vendored `axi_master_*_multiple_outstanding` (broken per §2 caveat). Implement N outstanding via own wrapper that calls channel-level `axi_master_write_aw` × N (distinct AXI IDs `i+1`), then `axi_master_write_w` × N in matched order, then `axi_master_write_b` × N to drain B; mirror for reads via `_read_ar` / `_read_r`. Each address gets per-task-tracked expected data, compared after both phases complete. | N ∈ {4, 8}; distinct IDs per outstanding; stresses NMU AW/AR FIFO + NSU MetaBuffer depth |
...
| G | `test_deep_outstanding_pressure_N{8,16}` | Same channel-level wrapper as C with N ∈ {8, 16} (16 = vendored-primitive cap from §2 caveat). Use a flag-polling watchdog (Verilator `--timing` restricts `disable` to enclosing blocks; named-fork cancellation is fragile): <br>`logic done = 0; fork begin : test_branch do_test_g(N); done = 1; end begin : wd_branch repeat (WATCHDOG_CYCLES) @(posedge ACLK); if (!done) $fatal(1, "G watchdog fired — stall ≠ deadlock evidence"); end join_any` | **Stall ≠ deadlock evidence** under credit-stub backpressure. Watchdog heuristic in §3.7 |
...
- Each wrapper records `expected[addr]` before issuing, then post-issue reads back via `axi_master_read_ar`/`_r` and compares; mismatch sets `error_flag=1`.
...
- `tb_genamba` builds under Verilator `--binary --timing` as a self-clocked top with DUT DPI + gen_amba VIP coexisting in one executable (coexistence proof).
