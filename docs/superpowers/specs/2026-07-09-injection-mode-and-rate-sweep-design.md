...
`cmodel_nsu_create` is widened rather than duplicated. The only hand-written caller is
`tests/wrap/test_cmodel_dpi.cpp:65`. The eleven `sim/tb/tb_top_*.sv` are **generated build artifacts**,
ignored by `.gitignore:54` and regenerated by `sim/verilator/Makefile:132` whenever `gen_tb_top.py`
changes. Editing the generator is sufficient; there is nothing to commit and nothing to force-add.
