...
Rationale: the class boundary follows the upstream RTL block it is a 1:1 port of
(`floo_vc_arbiter.sv`); splitting selection from drain would buy no test coverage the combined
...
`nsu::MetaBuffer` keeps the upstream RTL name (`floo_meta_buffer.sv`) and a field set --
`{src_id, upstream_id, rob_req, rob_idx}`, restored via peek-plus-commit -- wider than a plain
TSHR.
...
Rationale: the name matches its upstream RTL counterpart (`floo_wormhole_arbiter.sv`);
...
Rationale: risk identified, then designed (a VC-safe `(dst, id)` binding, cross-AI reviewed), then
...
Baseline: a per-hop turn-model VC assignment rule (upstream deprecated component,
`floo_vc_assignment.sv`). Ours: not ported -- the static `(dst, id)` fixed VC id (T-12) supersedes
...
Convention: public IP repos (upstream FlooNoC, pulp axi, OpenTitan) carry badges/citation files, a
