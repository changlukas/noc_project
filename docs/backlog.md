...
## In progress — checked-traffic-benchmark (branch `feat/checked-traffic-benchmark`, not pushed)
...
| 5 harness + deletes | **NEXT** | rewrite `matrix.yaml`/`run_regress.py` to drive both drivers (checker gate = grep scoreboard mismatch `$warning` / reorder_compare `$error`); **then D7 deletes**: `run_benchmark.py` + old co-sim YAML fan-out + `gen_test_patterns --from` path (KEEP AX4 base YAMLs — ctest consumes them, see Infra section). **Also reconcile `make check`**: its `run_benchmark.py` leg is dead at Stage-4 tip (probes unsuffixed `build/verilator/obj_dir_<topo>` but Makefile keys `_<RUN_CLASS>`; ignores `BUILD_ROOT`) — pre-existing, flagged by Stage-4 final review. Drop that leg from `check:` (keep lint+build+ctest) or route via `make sim`, and fix the `make help` text so it stops advertising a broken smoke |

Next-session start: read this backlog + BOTH specs (`2026-07-04-checked-traffic-benchmark-design.md` = Stages/decisions; `2026-07-05-benchmark-stage4-constrained-random-sim-interface-design.md` = the two-axis naming + unified interface), resume at **Stage 5**. Current run interface (Stage 4): **`make sim TB=tb_<topo> PATTERN=<pat> [SEED=]`** — PATTERN picks the axis (`neighbor/transpose/uniform_random/hotspot`=directed, `constrained_random`=CR); SEED unset draws+records a random seed. Env fixed on WSL/Linux (Verilator 5.048 + z3): **create a gitignored repo-root `local.mk` with `BUILD_ROOT := $(HOME)/noc_build` + `PYTHON3 := python3` + `VERILATOR := verilator`** (the `/mnt/e/.../build` tree is Windows-COFF, WSL `ld` rejects it; `build_config.mk` now `?=` so local.mk survives). yaml-cpp must be built once under `$HOME/noc_build` (see [[feedback_run_on_wsl_linux]]). Python emitter tests run on Windows `py -3` too. All benchmark work stays on `feat/checked-traffic-benchmark` (NOT pushed).
