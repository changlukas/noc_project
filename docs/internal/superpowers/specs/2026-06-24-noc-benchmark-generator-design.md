...
| `transpose` | 確定性 | `TransposeTrafficPattern`(`traffic.cpp:244`,id bit-half-swap)→ 方形下 (x,y)→(y,x) | guard:需偶次方方形 mesh(4x4 ✓);對角 (x,x)→self,booksim-faithful,預設 permit self;correctness smoke 不用它 |
...
- **guard**:`x_dim≤2^X_WIDTH`、`y_dim≤2^Y_WIDTH`、`x_dim×y_dim≤2^DST_ID_WIDTH`;transpose 需方形 mesh;違反即 fail-fast。
