...
- **destination**:`--pattern`(uniform/transpose/nearest_neighbor/hotspot)把 dst 編進 address。

組合:**benchmark run** = synthetic payload × 隨機/結構 pattern;**correctness 回歸** = conformity base(`--from`)× `nearest_neighbor`(確定性)。這完全取代 `gen_coordinate_scenarios`(刪除,無 wrapper);一支工具涵蓋兩種需求。
...
- §1 契約修正後:mesh_4x4_vc{1,2,4,8} co-sim 仍全綠(改用 `nearest_neighbor` 確定性分佈);destination 由 address 決定(可驗:改 scenario dst → 流量改向)。
...
- **契約修正觸及已 merge 的 gen_tb_top + 刪 gen_coordinate**:behavior-preserving 的 gate = 6 個 conformity scenario 在 `nearest_neighbor` 分佈下仍 scoreboard clean(覆蓋不變,非重現同樣流量)。call sites 須原子更新,否則回歸 runner 斷。
