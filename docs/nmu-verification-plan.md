...
| N0-SAM-11 | generate `mesh_2x2.yml` twice into separate temporary build roots | both packages are byte-identical, export exactly `SAM_NUM_RULES`, `SAM_MASK_SEL_WIDTH`, `sam_mask_sel_t`, `sam_idx_t`, `sam_rule_t`, and `SAM`, contain no `base_id`, and compile with `ni_sam` using only canonical generated widths |
