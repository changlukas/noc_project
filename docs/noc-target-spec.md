...
`rtl/README.md` follows the existing wrapper contract: NMU exposes the 58-bit `AWUSER`, NSU
exposes no USER port, and no other USER signal crosses either production top. The canonical child
`axi_aw_t`/`axi_w_t`/`axi_b_t`/`axi_ar_t`/`axi_r_t` records carry the complete payload, including
AWUSER in `axi_aw_t`.
...
**Header overhead and W/R utilization at fixed `NOC_ID_WIDTH = 3`.** Header overhead counts
...
As built in the C++ model: REQ and RSP 2 cycles per router, DAT 4 cycles per router for a head
and 3 for a body or tail, and on DAT 1 cycle at the source NI merge on injection plus 1 cycle at
the destination NI merge on ejection. REQ and RSP have no merge stage.
