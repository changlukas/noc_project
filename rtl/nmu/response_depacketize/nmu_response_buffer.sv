...
module nmu_response_buffer #(
    ...
    wire ni_flit_pkg::dat_flit_t [DAT_NUM_VC:0] r_data;
    ...
    cc_fifo #(.Depth(FIFO_DEPTH), .FallThrough(1'b0),
        .data_t(ni_flit_pkg::rsp_flit_t)) i_b_fifo (
        .clk_i, .rst_ni(1'b1), .flush_i(rst_i), .clr_i(1'b0),
        .full_o(b_full), .empty_o(b_empty), .usage_o(),
        .data_i(s_rsp_i), .push_i(s_rsp_valid_i && s_rsp_ready_o && is_b),
        .data_o(b_flit), .pop_i(m_b_valid_o && m_b_ready_i)
    );
    cc_fifo #(.Depth(FIFO_DEPTH), .FallThrough(1'b0),
        .data_t(ni_flit_pkg::rsp_flit_t)) i_r_fifo (
        .clk_i, .rst_ni(1'b1), .flush_i(rst_i), .clr_i(1'b0),
        .full_o(r_full), .empty_o(r_empty), .usage_o(),
        .data_i(s_rsp_i), .push_i(s_rsp_valid_i && s_rsp_ready_o && is_r),
        .data_o(narrow_r_flit), .pop_i(r_valid[0] && r_ready[0])
    );
    for (genvar vc = 0; vc < DAT_NUM_VC; vc++) begin : gen_dat_vc
        if (vc >= FIRST_RD_VC) begin : gen_read
            ...
            cc_fifo #(
                ...
                .data_i(s_dat_i), .push_i(dat_push[vc]),
    ...
    logic [DAT_NUM_VC-1:0] credit_return_reg, credit_return_next;
    assign credit_return_next = dat_pop;
    always_ff @(posedge clk_i) begin
        if (rst_i) credit_return_reg <= '0;
        else credit_return_reg <= credit_return_next;
    end
    assign dat_credit_return_o = rst_i ? '0 : credit_return_reg;

    // Hold only a stalled beat; RLAST does not lock the selected VC.
    rr_arb_tree #(
        .NumIn(DAT_NUM_VC+1), .DataType(ni_flit_pkg::dat_flit_t),
