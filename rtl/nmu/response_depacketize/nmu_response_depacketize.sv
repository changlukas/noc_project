...
module nmu_response_depacketize #(
    ...
    ni_child_types_pkg::nmu_b_response_t b;
    ni_child_types_pkg::nmu_r_response_t r;
    nmu_response_buffer #(
        .FIFO_DEPTH(FIFO_DEPTH), .DAT_NUM_VC(DAT_NUM_VC),
        .DAT_VC_MODE(DAT_VC_MODE), .DAT_RX_VC_DEPTH(DAT_RX_VC_DEPTH)
    ) i_buffer (
        .clk_i, .rst_i, .s_rsp_i, .s_rsp_valid_i, .s_rsp_ready_o,
        .s_dat_i, .s_dat_valid_i, .dat_credit_return_o,
        .m_b_o(b_flit), .m_b_valid_o, .m_b_ready_i,
        .m_r_o(r_flit), .m_r_valid_o, .m_r_ready_i
    );
    always_comb begin
        b = '0;
        r = '0;
