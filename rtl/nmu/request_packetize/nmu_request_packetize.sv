...
module nmu_request_packetize #(
    ...
    output wire ni_flit_pkg::req_flit_t              [ni_child_types_pkg::NMU_REQ_COUNT-1:0] m_req_o,
    output wire logic                                [ni_child_types_pkg::NMU_REQ_COUNT-1:0] m_req_valid_o,
    input  wire logic                                [ni_child_types_pkg::NMU_REQ_COUNT-1:0] m_req_ready_i,
    output wire ni_flit_pkg::dat_flit_t              [ni_child_types_pkg::NMU_DAT_COUNT-1:0] m_dat_o,
    output wire logic                                [ni_child_types_pkg::NMU_DAT_COUNT-1:0] m_dat_valid_o,
    input  wire logic                                [ni_child_types_pkg::NMU_DAT_COUNT-1:0] m_dat_ready_i
);
    ...
    ni_flit_pkg::req_flit_t [NMU_REQ_COUNT-1:0] req_flit;
    ni_flit_pkg::dat_flit_t [NMU_DAT_COUNT-1:0] dat_flit;
    assign m_req_valid_o[NMU_REQ_AW] = !rst_i && !narrow_aw_empty;
    assign m_req_valid_o[NMU_REQ_W]  = !rst_i && !narrow_w_empty;
    assign m_req_valid_o[NMU_REQ_AR] = !rst_i && !ar_empty;
    assign m_dat_valid_o[NMU_DAT_AW] = !rst_i && !data_aw_empty;
    assign m_dat_valid_o[NMU_DAT_W]  = !rst_i && !data_w_empty;
    for (genvar i = 0; i < NMU_REQ_COUNT; i++) begin : gen_req
        assign m_req_o[i] = m_req_valid_o[i] ? req_flit[i] : '0;
    end
    for (genvar i = 0; i < NMU_DAT_COUNT; i++) begin : gen_dat
        assign m_dat_o[i] = m_dat_valid_o[i] ? dat_flit[i] : '0;
    end
    always_comb begin
        req_flit                    = '0;
        dat_flit                    = '0;
        req_flit[NMU_REQ_AW].header = make_header(ni_flit_pkg::AXI_CH_WIDTH'(ni_flit_pkg::AXI_CH_NarrowAw),
            narrow_aw_head.meta, narrow_aw_head.collective_op, narrow_aw_head.collective_mask,
            '0, !narrow_aw_head.meta.ordering_req, 1'b0);
        req_flit[NMU_REQ_AW].payload = ni_flit_pkg::AW_WIDTH'(pack_aw(narrow_aw_head));
        req_flit[NMU_REQ_W].header   = make_header(ni_flit_pkg::AXI_CH_WIDTH'(ni_flit_pkg::AXI_CH_NarrowW),
        ...
        req_flit[NMU_REQ_W].payload = ni_flit_pkg::AW_WIDTH'(pack_w(narrow_w_head, 1'b1));
        req_flit[NMU_REQ_AR].header = make_header(ni_flit_pkg::AXI_CH_WIDTH'(
            ar_head.meta.route.domain.is_data ? ni_flit_pkg::AXI_CH_DataAr : ni_flit_pkg::AXI_CH_NarrowAr),
            ar_head.meta, '0, '0, '0, 1'b0, 1'b1);
        req_flit[NMU_REQ_AR].payload = ni_flit_pkg::AW_WIDTH'(pack_ar(ar_head));
        dat_flit[NMU_DAT_AW].header  = make_header(ni_flit_pkg::AXI_CH_WIDTH'(ni_flit_pkg::AXI_CH_DataAw),
            data_aw_head.meta, data_aw_head.collective_op, data_aw_head.collective_mask,
            '0, !data_aw_head.meta.ordering_req, 1'b0);
        dat_flit[NMU_DAT_AW].payload = pack_aw(data_aw_head);
        dat_flit[NMU_DAT_W].header   = make_header(ni_flit_pkg::AXI_CH_WIDTH'(ni_flit_pkg::AXI_CH_DataW),
        ...
        dat_flit[NMU_DAT_W].payload = pack_w(data_w_head, 1'b0);
    end
    cc_fifo #(
        .Depth       (FIFO_DEPTH                          ),
        .FallThrough (1'b0                                ),
    ...
    cc_fifo #(
        ...
        .pop_i   (m_req_valid_o[NMU_REQ_AW] && m_req_ready_i[NMU_REQ_AW])
    );
    ...
    cc_fifo #(
        ...
        .push_i  (aw_accept && aw_is_data                               ),
        .data_o  (data_aw_head                                          ),
        .pop_i   (m_dat_valid_o[NMU_DAT_AW] && m_dat_ready_i[NMU_DAT_AW])
    );
    ...
    cc_fifo #(
        ...
        .push_i  (ar_accept                                             ),
        .data_o  (ar_head                                               ),
        .pop_i   (m_req_valid_o[NMU_REQ_AR] && m_req_ready_i[NMU_REQ_AR])
    );
    ...
    cc_fifo #(
        ...
        .pop_i   (m_req_valid_o[NMU_REQ_W] && m_req_ready_i[NMU_REQ_W])
    );
    ...
    cc_fifo #(
        ...
        .push_i  (w_accept && owner_head.meta.route.domain.is_data    ),
        .data_o  (data_w_head                                         ),
        .pop_i   (m_dat_valid_o[NMU_DAT_W] && m_dat_ready_i[NMU_DAT_W])
    );
