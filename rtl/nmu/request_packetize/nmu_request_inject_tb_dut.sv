...
module nmu_request_inject_tb_dut #(
    ...
    nmu_request_packetize #(.FIFO_DEPTH(FIFO_DEPTH), .SRC_ID(SRC_ID), .SRC_PORT_ID(SRC_PORT_ID)) i_packetize (
        .clk_i, .rst_i, .s_aw_i, .s_aw_valid_i, .s_aw_ready_o,
        .s_w_i, .s_w_valid_i, .s_w_ready_o, .s_ar_i, .s_ar_valid_i, .s_ar_ready_o,
    ...
    nmu_channel_assign #(.DAT_NUM_VC(DAT_NUM_VC), .DAT_VC_MODE(DAT_VC_MODE),
        .ROUTER_VC_DEPTH(ROUTER_VC_DEPTH)) i_channel_assign (
        .clk_i, .rst_i, .s_req_i(req_candidates), .s_req_valid_i(req_valid), .s_req_ready_o(req_ready),
        .s_dat_i(dat_candidates), .s_dat_valid_i(dat_valid), .s_dat_ready_o(dat_ready),
        .m_req_o, .m_req_valid_o, .m_req_ready_i, .m_dat_o, .m_dat_valid_o, .dat_credit_return_i
    );
endmodule
`resetall
