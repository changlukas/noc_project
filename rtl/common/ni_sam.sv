...
module ni_sam #(
    ...
    output wire sam_result_t  sam_idx_o,
    output wire logic         lookup_valid_o,
    output wire logic         lookup_error_o
);

    localparam int unsigned SAM_MASK_SEL_FIELD_W =
        $clog2(ni_params_pkg::AXI_ADDR_WIDTH + 1);
    localparam int unsigned SAM_RESULT_W =
    ...
    if ($bits(addr_t) != ni_params_pkg::AXI_ADDR_WIDTH) begin : gen_invalid_addr_type
        initial $fatal(0, "Error: addr_t must match AXI_ADDR_WIDTH (instance %m)");
    end

    if ($bits(sam_mask_sel_t) != 2 * SAM_MASK_SEL_FIELD_W) begin : gen_invalid_mask_selector_type
        initial $fatal(0, "Error: sam_mask_sel_t has an incompatible width (instance %m)");
    end

    if ($bits(sam_result_t) != SAM_RESULT_W) begin : gen_invalid_index_type
        initial $fatal(0, "Error: sam_result_t has an incompatible width (instance %m)");
    end

    if ($bits(sam_rule_t) != $bits(sam_result_t) + 2 * $bits(addr_t)) begin : gen_invalid_rule_type
