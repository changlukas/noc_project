...
```bash
py -3 specgen/tools/codegen.py --target {cpp|sv} --domain {packet|signals|registers} [--out PATH]
...
The eight `(target, domain)` combinations and their outputs:
...
| `cpp` | `registers` | `ni_regs.h` | `generated/json/ni_registers.json` |
...
| `sv` | `registers` | `ni_regs_pkg.sv` | `generated/json/ni_registers.json` |
...
Re-elaborates all eight artifacts into a scratch dir, diffs each against the committed `generated/cpp/` and `generated/sv/`, and prints a unified diff (up to 40 lines per file) for anything that drifted.
