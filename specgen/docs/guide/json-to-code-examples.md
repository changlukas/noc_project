# From MD spec to generated C++/SV

以 generic placeholder 展示 `spec_validate` 三條 codegen 通路 (packet / signals / registers)
的「authored MD → JSON → elaborated header」轉換 pattern。
...
generated/ni_*.json
        ...
        │  tools/codegen.py
        ▼
include/ni_*.h   +  rtl_pkg/ni_*_pkg.sv   (byte-identical 雙產物)
```
...
**MD source** (table row in `docs/design/02_flit.md`):

```markdown
| <FIELD_NAME> | `<WIDTH_PARAM>` | [<msb>:<lsb>] | <stage> | <description> |
```

`<WIDTH_PARAM>` 可為 single symbol 或 expression (e.g. `<P1> + <P2>`)。
`[<msb>:<lsb>]` 為 MD 註解，不進入 JSON。

**JSON excerpt** (`generated/ni_packet.json`, purely symbolic):
...
**Elaborated `rtl_pkg/ni_*_pkg.sv`**:
...
目前 spec 中的實際 header fields 可在 `generated/ni_packet.json` 查到。
...
**JSON pin entry** (`generated/ni_signals.json`, pin 已無 `default`):
...
目前 spec 中的實際 signal pins 與 interface port_parameters 可在 `generated/ni_signals.json` 查到。
...
**JSON register entry** (`generated/ni_registers.json`):
...
`access`: `RO` / `RW` / `RW1C` / `WO` (見 `include/ni_regs.h` 的 `AccessMode` enum)。無 sub-field 的 register 省略 `fields` 鍵。
...
目前 spec 中的實際 registers 可在 `generated/ni_registers.json` 查到。
