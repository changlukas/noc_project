...
TEST_F(AxiProtocolDeath, BurstEncoding_RejectsInvalid) {
    ...
    EXPECT_DEATH(
        {
            AXI_PROTOCOL_ASSERT(rules::check_burst_encoding(static_cast<axi::Burst>(3)),
                                "BURST_ENCODING");
        },
        ".*");
}
...
TEST_F(AxiProtocolDeath, WrapAlign_RejectsUnaligned) {
    ...
    EXPECT_DEATH(
        {
            AXI_PROTOCOL_ASSERT(rules::check_wrap_align(axi::Burst::WRAP, 0x1001, 2), "WRAP_ALIGN");
        },
        ".*");
}
...
TEST_F(AxiProtocolDeath, RespEncoding_RejectsInvalid) {
    ...
    EXPECT_DEATH(
        {
            AXI_PROTOCOL_ASSERT(rules::check_resp_encoding(static_cast<axi::Resp>(4)),
                                "RESP_ENCODING");
        },
        ".*");
}
...
TEST_F(AxiProtocolDeath, WLastTiming_RejectsEarlyLast) {
    ...
    // must trip.
    EXPECT_DEATH(
        { AXI_PROTOCOL_ASSERT(rules::check_w_last_timing(true, 1, 3), "W_LAST_TIMING"); }, ".*");
}
...
TEST_F(AxiProtocolDeath, StrbSparseLegal_RejectsBitsOutsideWindow) {
    ...
    EXPECT_DEATH(
        {
            AXI_PROTOCOL_ASSERT(rules::check_strb_sparse_legal(0x0000'0010u, /*size=*/2,
                                                               /*beat_addr=*/0x1000),
                                "STRB_SPARSE_LEGAL");
        },
        ".*");
...
TEST_F(AxiProtocolDeath, Cross4kb_RejectsIncrCrossing) {
    ...
    EXPECT_DEATH(
        {
            AXI_PROTOCOL_ASSERT(
                rules::check_4kb_cross(0x0FE0, /*len=*/1, /*size=*/5, axi::Burst::INCR),
                "CROSS_4KB");
        },
        ".*");
}
...
TEST_F(AxiProtocolDeath, ExclusivePow2_RejectsLen2) {
    ...
    EXPECT_DEATH(
        {
            AXI_PROTOCOL_ASSERT(rules::check_exclusive_total_pow2(axi::LockType::Exclusive, 2),
...
TEST_F(AxiProtocolDeath, ExclusiveAlign_RejectsUnaligned) {
    ...
    EXPECT_DEATH(
        {
            AXI_PROTOCOL_ASSERT(rules::check_exclusive_addr_aligned_to_total(
...
TEST_F(AxiProtocolDeath, ExclusiveBurstFixed_Rejects) {
    ...
    EXPECT_DEATH(
        {
            AXI_PROTOCOL_ASSERT(
                rules::check_exclusive_burst_not_fixed(axi::LockType::Exclusive, axi::Burst::FIXED),
...
TEST_F(AxiProtocolDeath, ExclusiveValid_INCR_1Beat_Size5) {
    ...
    EXPECT_TRUE(
        rules::check_exclusive_addr_aligned_to_total(axi::LockType::Exclusive, 0x1000, 0, 5));
    EXPECT_TRUE(rules::check_exclusive_burst_not_fixed(axi::LockType::Exclusive, axi::Burst::INCR));
}
...
TEST_F(AxiProtocolDeath, ExclusiveValid_WRAP_4Beat_Size5) {
    ...
    EXPECT_TRUE(
        rules::check_exclusive_addr_aligned_to_total(axi::LockType::Exclusive, 0x1000, 3, 5));
    EXPECT_TRUE(rules::check_exclusive_burst_not_fixed(axi::LockType::Exclusive, axi::Burst::WRAP));
}
