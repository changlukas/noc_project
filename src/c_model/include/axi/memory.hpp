...
class Memory : public IMemoryPort {
  public:
    Memory(uint64_t base_addr, std::size_t size_bytes, std::size_t write_latency_ticks,
           std::size_t read_latency_ticks, std::size_t pending_queue_depth = 32,
           uint8_t fill_byte = 0x00)
        : base_(base_addr),
          size_(size_bytes),
          write_lat_(write_latency_ticks),
          read_lat_(read_latency_ticks),
    ...
    std::optional<MemWriteResp> pop_write_resp() override {
        ...
        auto r = write_resp_q_.front();
        write_resp_q_.pop_front();
        return r;
    }
    std::optional<MemReadResp> pop_read_resp() override {
        if (read_resp_q_.empty()) return std::nullopt;
        auto r = read_resp_q_.front();
        read_resp_q_.pop_front();
        return r;
    }
  ...
  private:
    ...
    MemReadResp perform_read_(const MemReadReq& req) {
        ...
        resp.last = req.last;
        const uint64_t aligned_word = req.addr & ~static_cast<uint64_t>(DATA_BYTES - 1);
        const std::size_t byte_lane = static_cast<std::size_t>(req.addr & (DATA_BYTES - 1));
        ...
        for (std::size_t i = 0; i < copy_bytes; ++i) {
            resp.data[byte_lane + i] = storage_[(aligned_word - base_) + byte_lane + i];
        }
        return resp;
    }
