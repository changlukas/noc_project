...
TEST_P(NmuVcArbParam, ReadWriteSplit_AW_AR_GoSeparateVcs) {
    ...
    SCENARIO("NMU VcArb Mode A: AW -> write_vc=0, AR -> read_vc=1; "
             "verify pending queues + downstream stamps");
    LoopbackNoc noc(/*req*/32, /*rsp*/32);
    auto arb = VcArb::read_write_split(noc.req_out(), num_vc,
...
TEST_P(NmuVcArbParam, MultiCandidate_HoLAvoidance) {
    ...
    SCENARIO("NMU VcArb Mode B: saturate first AW candidate VC -> next AW "
    ...
    auto arb = VcArb::multi_candidate(noc.req_out(), num_vc,
...
TEST_P(NmuVcArbParam, WFollowsAW_InvariantEnforced) {
    ...
    SCENARIO("NMU VcArb Mode B: AW burst spills across candidate VCs; "
...
TEST_P(NmuVcArbParam, WlastFromPayloadNotHeader) {
    ...
    LoopbackNoc noc(/*req*/64, /*rsp*/64);
    auto arb = VcArb::read_write_split(noc.req_out(), num_vc, 0, 1);
    ASSERT_TRUE(arb.push_flit(make_flit(ni::AXI_CH_AW)));
...
TEST_P(NmuVcArbParam, RoundRobinFairness_AllVcsServiced_NoStarvation) {
    ...
    SCENARIO("NMU VcArb Mode B: num_vc ARs pre-routed to distinct VCs via "
...
TEST_P(NmuVcArbParam, CreditGating_TickIdleWhenAllVcsBlocked) {
    ...
    SCENARIO("NMU VcArb: all flits -> VC=0; LoopbackNoc per_vc_depth=1 caps "
    ...
    auto arb = VcArb::read_write_split(noc.req_out(), num_vc, 0, 0,
                                       /*pending_depth=*/8);
    ASSERT_TRUE(arb.push_flit(make_flit(ni::AXI_CH_AR)));
    ASSERT_TRUE(arb.push_flit(make_flit(ni::AXI_CH_AR)));
    ASSERT_TRUE(arb.push_flit(make_flit(ni::AXI_CH_AR)));
...
// Backpressure: VcArb pending_depth=2, single VC. After 2 pushes, pending
...
TEST_P(NmuVcArbParam, BackpressureChain_VcArbToUpstream) {
    ...
    SCENARIO("NMU VcArb: VcArb pending_depth=2. After 2 pushes, "
             "VcArb's pending_[0] is full -> push_flit returns false; "
